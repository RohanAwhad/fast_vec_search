...
class PyExaAISearch(BaseSearch):
  def __init__(self,
    ...
    self.ranker = Reranker("answerdotai/answerai-colbert-small-v1", model_type='colbert', max_length=512)
    # self.ranker = Reranker('ms-marco-TinyBERT-L-2-v2', model_type='flashrank', max_length=512)
  ...
  def search(self,
    ...
    exaai_collection = ExaAIDB(embedding_size=256, subvector_size=8, n_centroids=10, min_cluster_size=100_000).create_collection('test')
    exaai_collection.add(ids=corpus_ids, documents=corpus, embeddings=corpus_embeddings)
    exaai_index = exaai_collection.create_index()
    ...
    # results = {}
    # from tqdm import tqdm
    # for qid, query_embedding in tqdm(zip(query_ids, query_embeddings), total=len(query_ids)):
    ...
    #
    #   # rerank
    #   docs = [corpus[corpus_ids[idx]]['text'] for idx in map(int, global_top_k)]
    #   query = queries[qid]
    #   reranker_results = self.ranker.rank(query=query, docs=docs)
    #   # print('done')
    #
    #   candidate_scores = [x.score for x in reranker_results]
    #   final_local_ids  = [x.document.doc_id for x in reranker_results]
    #   final_global_ids = np.array(global_top_k)[final_local_ids]
    #
    #   # final_local_ids = local_top_k
    #   # final_global_ids = global_top_k
    #
    #
    #   results[qid] = {
    #     corpus_ids[idx]: float(candidate_scores[local_idx])
    #     for local_idx, idx in zip(final_local_ids, final_global_ids)
    #   }
    #
    # return results
